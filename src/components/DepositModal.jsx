...
export default function DepositModal({ isOpen, onClose, defaultTabType = 'deposit' }) {
  if (!isOpen) return null;

  const { selectedFiat, fiatSymbol, submitDepositRequest, submitWithdrawalRequest, cryptoBalances } = useBet();
  const [modalMode, setModalMode] = useState(defaultTabType);
  ...
  const handleWithdrawalSubmit = (e) => {
    e.preventDefault();
    if (!amount || parseFloat(amount) <= 0 || !withdrawalAddress) return;

    const assetBalance = cryptoBalances[withdrawalAsset.toLowerCase()];
    ...
    setTimeout(() => {
      ...
      const methodLabel = `Crypto (${withdrawalAsset} - ${withdrawalAsset === 'USDT' ? withdrawalNetwork : 'Native'})`;
      const res = submitWithdrawalRequest(methodLabel, withdrawalAddress, amount);
      ...
      >
        ...
        <div style={{
          ...
          <h3 style={{ fontSize: '1.1rem', display: 'flex', alignItems: 'center', gap: '8px', color: '#ffffff' }}>
            {modalMode === 'deposit' ? (
              ...
              <ArrowRightLeft size={20} style={{ color: 'var(--brand-yellow)' }} />
            )}
        ...
        {verifying && (
          <div style={{
            ...
            <h4 style={{ color: '#ffffff' }}>{modalMode === 'deposit' ? 'Routing to P2P Agent...' : 'Processing Node Transfer...'}</h4>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>
              {modalMode === 'deposit' 
                ? 'Transmitting your deposit receipt details to the selected P2P cashier agent. Please hold on...' 
                : 'Signing blockchain withdrawal transaction. Request routing to node pool...'}
            </p>
          </div>
        )}
        ...
        {!verifying && success && (
          <div style={{
            ...
            <h4 style={{ color: '#ffffff', fontSize: '1.2rem' }}>{modalMode === 'deposit' ? 'Request Submitted!' : 'Withdrawal Pending!'}</h4>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>
              {modalMode === 'deposit' 
                ...
                : (successMsg || `Your withdrawal request has been submitted. Agent release will be credited to your address soon.`)}
            </p>
          </div>
        )}
        ...
        {!verifying && !success && (
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            {/* Top Level Mode Selector (Deposit vs Withdrawal Toggle) */}
            <div style={{ display: 'flex', background: 'rgba(0,0,0,0.25)', borderBottom: '1px solid var(--border-color)' }}>
              <button
                type="button"
                onClick={() => { setModalMode('deposit'); setAmount(''); }}
                style={{
                  flex: 1,
                  padding: '12px 0',
                  background: 'none',
                  border: 'none',
                  borderBottom: modalMode === 'deposit' ? '3px solid var(--brand-emerald)' : '3px solid transparent',
                  color: modalMode === 'deposit' ? 'var(--brand-yellow)' : 'var(--text-muted)',
                  fontWeight: 'bold',
                  fontSize: '0.82rem',
                  cursor: 'pointer',
                  transition: 'all 0.2s'
              ...
              <button
                type="button"
                onClick={() => { setModalMode('withdraw'); setAmount(''); }}
                style={{
                  flex: 1,
                  padding: '12px 0',
                  background: 'none',
                  border: 'none',
                  borderBottom: modalMode === 'withdraw' ? '3px solid var(--brand-emerald)' : '3px solid transparent',
                  color: modalMode === 'withdraw' ? 'var(--brand-yellow)' : 'var(--text-muted)',
                  fontWeight: 'bold',
                  fontSize: '0.82rem',
                  cursor: 'pointer',
                  transition: 'all 0.2s'
            ...
            {modalMode === 'withdraw' ? (
              // B: Withdrawal Mode form
              <form onSubmit={handleWithdrawalSubmit} style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                
                {/* Select Asset */}
                <div>
                  <label style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '8px', display: 'block', fontWeight: 'bold' }}>
                    Select Asset to Withdraw
                  </label>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {['USDT', 'BTC', 'ETH'].map((asset) => {
                      const isSel = withdrawalAsset === asset;
                      return (
                        <button
                          key={asset}
                          type="button"
                          onClick={() => { setWithdrawalAsset(asset); setAmount(''); }}
                          style={{
                            flex: 1,
                            padding: '10px 0',
                            backgroundColor: isSel ? 'rgba(198, 255, 0, 0.08)' : 'var(--bg-card)',
                            border: isSel ? '2px solid var(--brand-yellow)' : '1px solid var(--border-color)',
                            borderRadius: '8px',
                            color: isSel ? 'var(--brand-yellow)' : '#ffffff',
                            fontWeight: 'bold',
                            cursor: 'pointer',
                            display: 'flex',
                            flexDirection: 'column',
                        ...
                        >
                          ...
                          <span style={{ fontSize: '0.75rem' }}>{asset}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                {/* Select Network (USDT only) */}
                {withdrawalAsset === 'USDT' && (
                  <div>
                    <label style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '6px', display: 'block', fontWeight: 'bold' }}>
                      Select Network
                    </label>
                    <div style={{ display: 'flex', gap: '6px' }}>
                      {['TRC20', 'ERC20', 'BSC'].map((net) => {
                        const isSel = withdrawalNetwork === net;
                        return (
                          <button
                            key={net}
                            type="button"
                            onClick={() => setWithdrawalNetwork(net)}
                            style={{
                              flex: 1,
                              padding: '6px 8px',
                              backgroundColor: isSel ? 'rgba(198, 255, 0, 0.05)' : 'transparent',
                              border: `1.5px solid ${isSel ? 'var(--brand-yellow)' : 'var(--border-color)'}`,
                              borderRadius: '6px',
                              color: isSel ? 'var(--brand-yellow)' : '#ffffff',
                              fontSize: '0.7rem',
                              fontWeight: 600,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '0.72rem', color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 'bold' }}>
                    Destination Wallet Address ({withdrawalAsset === 'USDT' ? withdrawalNetwork : 'Native'})
                  </label>
                  <input
                    type="text"
                    required
                    placeholder={`Enter your blockchain ${withdrawalAsset} address`}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '0.7rem', color: 'var(--brand-emerald)', fontWeight: 'bold' }}>
                      Available: {cryptoBalances[withdrawalAsset.toLowerCase()].toFixed(4)} {withdrawalAsset}
                    </span>
                  </div>
                  <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                    ...
                    />
                    <span style={{ position: 'absolute', right: '14px', fontWeight: 'bold', fontSize: '0.85rem', color: 'var(--brand-yellow)' }}>
                      {withdrawalAsset}
                    </span>
                  </div>
                  {/* Live Validation Alert */}
                  {amount && parseFloat(amount) > cryptoBalances[withdrawalAsset.toLowerCase()] && (
                    <span style={{ fontSize: '0.68rem', color: 'var(--live-red)', fontWeight: 'bold', marginTop: '2px' }}>
                      ⚠️ Insufficient balance. Available is only {cryptoBalances[withdrawalAsset.toLowerCase()].toFixed(4)} {withdrawalAsset}.
                    </span>
                  )}
                </div>
                ...
                <button
                  type="submit"
                  disabled={!amount || parseFloat(amount) <= 0 || parseFloat(amount) > cryptoBalances[withdrawalAsset.toLowerCase()] || !withdrawalAddress}
                  ...
                  style={{
                    ...
                    color: '#080a0f',
                    boxShadow: '0 4px 14px rgba(198, 255, 0, 0.25)',
                    fontWeight: 700,
                    marginTop: '10px',
                    opacity: (!amount || parseFloat(amount) <= 0 || parseFloat(amount) > cryptoBalances[withdrawalAsset.toLowerCase()] || !withdrawalAddress) ? 0.5 : 1,
                    cursor: (!amount || parseFloat(amount) <= 0 || parseFloat(amount) > cryptoBalances[withdrawalAsset.toLowerCase()] || !withdrawalAddress) ? 'not-allowed' : 'pointer'
                  }}
                >
